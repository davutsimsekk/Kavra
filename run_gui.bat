...
if not exist "%ROOT%webui\dist\index.html" (
  echo Modern masaustu arayuzu ilk kez hazirlaniyor...
...
cd /d "%ROOT%"
"%ROOT%venv\Scripts\python.exe" -m studio_web.desktop
if errorlevel 1 goto :error
goto :eof
:error
echo Masaustu arayuzu baslatilamadi. Yukaridaki hata mesajini kontrol et.
pause
